...
def open_file() -> IO:
    # Add comments
    file_name = input("Enter a filename: ") # asking for user file name
    file_pointer = None # creating a file pointer
    while file_pointer is None: # keeps looping until proper file name is given
        try:
            file_pointer = open(file_name, "r") # opening the file given
        except IOError:
            print("Error in filename.") # reasking for file name is the file name is not valid
...
def read_file(fp: IO) -> List[List[int]]:
    ...
    n = int(n) # first value is the number of users in the social network
    ...
    while line is not None and len(line) >= 3: # checking the line for specific parameters
        ...
        network[int(split_line[0])].append(int(split_line[1])) # get the user id values to hold the friends for each values
...
def main():
    ...
    while True:
        try:
            ...
            print("The suggested friend for " + str(user) + " is " + str(friend))
